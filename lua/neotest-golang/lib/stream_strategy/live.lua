...
---Create a live streaming data source for production file-based streaming
...
function M.create_stream(json_filepath)
  ...
  if json_filepath ~= nil then
    logger.debug("Setting up live streaming for file: " .. json_filepath)
    neotest_lib.files.write(json_filepath, "")
    return neotest_lib.files.stream_lines(json_filepath)
  else
    local error_msg = "JSON filepath is required for live streaming"
