...
- Some terminals/environments will write inject linebreaks and even truncate
    ...
    impossible to reliably parse the output as JSON. This is a widespread
    problem and to mitigate it, use the [`gotestsum` runner](config.md#runner)
    instead. Gotestsum writes all JSON test output to file, which is a lot more
    reliable. This particular problem has been reported to happen in some
    specific cases: - When on Windows:
