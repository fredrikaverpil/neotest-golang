...
describe("Integration: testify suites with non-contiguous methods", function()
  it("handles methods with gaps >20 lines correctly", function()
    -- ===== ARRANGE =====
    ...
    local position_id = vim.uv.cwd()
      .. "/tests/features/internal/testifysuites/gaps_test.go"
    ...
    local want = {
      results = {
        ...
        [position_id .. "::TestGapSuite"] = {
          status = "passed",
          errors = {},
        },
        -- Individual test method results
        -- TestFirst and TestSecond are contiguous (within 20 lines)
        [position_id .. "::TestGapSuite::TestFirst"] = {
          status = "passed",
          errors = {},
        },
        [position_id .. "::TestGapSuite::TestSecond"] = {
          status = "passed",
          errors = {},
        },
        -- TestThird has >20 line gap from TestSecond, but should still run
        -- as part of the suite (ID includes TestGapSuite namespace)
        [position_id .. "::TestGapSuite::TestThird"] = {
          status = "passed",
          errors = {},
        },
        -- TestFourth is contiguous with TestThird
        [position_id .. "::TestGapSuite::TestFourth"] = {
          status = "passed",
          errors = {},
        },
      },
    ...
    assert.are.same(
      vim.inspect(want),
      vim.inspect(got),
      "All test methods should run successfully including non-contiguous ones"
    )
  end)
end)
